	...
	if (level == 1) {
		...
		for (int i = 0; i < 5; ++i) {
			...
			if (!Boss.projectils[i].getActive()) {
				...
				switch (type) {
				case 1:
					Boss.projectils[i].setDimensions(80, 40);
					break;
				case 2:
...
void cGame::RenderEnemies(int id1, int id2, int id3)
{
	...
	//if(this->isVisible(x)) {
	if (level == 1) {
		Boss.Draw1(Data.GetID(IMG_BOSS1));
		for (int i = 0; i < 5; ++i) {
			if (Boss.projectils[i].getActive()) {
				Boss.projectils[i].DrawRectBoss1(Data.GetID(IMG_MISSILES_BOSS));
			}
		}
	}
		else if (level == 2) {
...
bool cGame::generateEnemies(int level)
{
	...
	if(level == 1)
		...
		res = res = Data.LoadImage(IMG_MISSILES_BOSS, "img/enemies/projectilsBoss1.png", GL_RGBA);
		if (!res) return false;
	else if (level == 2)
	{
